...
function CheckForCode(request, response, next) {
    if (!request.query.code) {
        response.redirect(
            `https://discord.com/api/oauth2/authorize?client_id=1209270223593799800&response_type=code&redirect_uri=${config.server.EndpointUriEncoded}%2Fauth%2Flogin&scope=identify`
        );
        return;
    }
...
async function GetToken(request, response, next) {
    ...
    const auth_request = new Request("https://discord.com/api/oauth2/token", {
        ...
        body: new URLSearchParams({
            ...
            redirect_uri: `${config.server.Endpoint}/auth/login`,
            scope: "identify",
        }).toString(),
        headers: {
...
async function GetUserData(request, response, next) {
    ...
    response.redirect(`${config.server.Endpoint}/`);
}
...
async function Logout(request, response, next) {
    ...
    const auth_request = new Request(
...
DiscordAuthRouter.use("/login", CheckForCode, GetToken, GetUserData);
